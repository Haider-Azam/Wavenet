...
#*    Original Source
#*    Title: WaveNet
#*    Author: golbin (Jin Kim)    
#*    Availability: https://github.com/golbin/WaveNet
#*
#***************************************************************************************/
"""
...
class DilatedCausalConv1d(torch.nn.Module):
    ...
    def __init__(self, channels, dilation=3,kernel_size=3):
        super(DilatedCausalConv1d, self).__init__()
        self.dilation=dilation
        self.kernel_size=kernel_size
        self.conv = torch.nn.Conv1d(channels, channels,
                                    kernel_size=kernel_size, stride=1,  # Fixed for WaveNet
    ...
    def forward(self, x):
        #print(f'dilation input:{x.shape}')
        #print(f'padding:{self.dilation*(self.kernel_size-1)}')
        x=torch.nn.functional.pad(x,(self.dilation*(self.kernel_size-1),0))
        output = self.conv(x)#[:, :, :-self.dilation]
        #print(f'dilation output:{output.shape}')
...
class ResidualBlock(torch.nn.Module):
    def __init__(self, res_channels, skip_channels, dilation, kernel_size=3):
        """
        Residual block
        ...
        self.dilated = DilatedCausalConv1d(res_channels, dilation=dilation,kernel_size=kernel_size)
...
class ResidualStack(torch.nn.Module):
    def __init__(self, layer_size, stack_size, res_channels, skip_channels,dilation_rate=2,kernel_size=3):
        """
    ...
    def _residual_block(res_channels, skip_channels, dilation,kernel_size):
        block = ResidualBlock(res_channels, skip_channels, dilation, kernel_size=kernel_size)
    ...
    def build_dilations(self):
        ...
        for s in range(0, self.stack_size):
            ...
            for l in range(0, self.layer_size):
                dilations.append(self.dilation_rate ** l)
    ...
    def stack_res_block(self, res_channels, skip_channels):
        ...
        for dilation in dilations:
            block = self._residual_block(res_channels, skip_channels, dilation,kernel_size=self.kernel_size)
    ...
    def forward(self, x, skip_size):
        ...
        for res_block in self.res_blocks:
            ...
            output = res_block(output, skip_size)
...
class WaveNet(torch.nn.Module):
    def __init__(self, layer_size, stack_size, in_channels, res_channels,dilation_rate=2,kernel_size=3):
        """
        Stack residual blocks by layer and stack size
        :param layer_size: integer, 10 = layer[dilation=1, dilation=2, 4, 8, 16, 32, 64, 128, 256, 512]
        :param stack_size: integer, 5 = stack[layer1, layer2, layer3, layer4, layer5]
        ...
        self.res_stack = ResidualStack(layer_size, stack_size, res_channels, in_channels,dilation_rate=self.dilation_rate,
